...
class Logger
{
    ...
    public function apiLog(string $platform, string $action, array $data = [])
    {
        $message = "API Call - Platform: {$platform}, Action: {$action}";
        $this->log('info', $message, $data);
    }

    public function syncLog(string $accountId, string $status, array $details = [])
    {
        $message = "Data Sync - Account: {$accountId}, Status: {$status}";
        $this->log('info', $message, $details);
    }

    public function billingLog(string $clientId, string $action, array $data = [])
    {
    ...
    private function getLogFilename(string $level)
    {
        $date = date('Y-m-d');
     ...
     * ログファイルのローテーション
     */
    public function rotateLogFiles(int $keepDays = 30)
    {
        $deletedCount = 0;
     ...
     * ログファイルのサイズを取得
     */
    public function getLogFileSize(string $level = null)
    {
        if ($level) {
            $filename = $this->getLogFilename($level);
