...
class Client extends BaseModel
{
    ...
    {
        return $this->findAllBy(array('is_active' => true), array('company_name' => 'ASC'));
    }

    /**
     * クライアントの広告アカウントを取得
     */
    public function getAdAccounts(int $clientId)
    {
        $adAccountModel = new AdAccount();
        return $adAccountModel->findAllBy(array('client_id' => $clientId, 'is_active' => true));
    }

    /**
     * クライアントの手数料設定を取得
     */
    public function getFeeSettings(int $clientId)
    {
     ...
     * クライアントの月次集計データを取得
     */
    public function getMonthlySummaries(int $clientId, string $yearMonth = null)
    {
     ...
     * クライアントの請求書一覧を取得
     */
    public function getInvoices(int $clientId, int $limit = null)
    {
        $invoiceModel = new Invoice();
        return $invoiceModel->findAllBy(array('client_id' => $clientId), array('created_at' => 'DESC'), $limit);
    }

    /**
     * クライアントの現在の月次パフォーマンスを取得
     */
    public function getCurrentMonthPerformance(int $clientId)
    {
        ...
        $result = $this->query($sql, array($clientId, $currentMonth));
     ...
     */
    public function getDailyDataForPeriod(int $clientId, string $startDate, string $endDate)
    {
        $sql = "SELECT 
                    dad.date_value,
                    aa.platform,
     ...
     */
    public function getPlatformPerformance(int $clientId, string $yearMonth = null)
    {
        ...
        return $this->query($sql, array($clientId, $yearMonth));
    }

    /**
     * 契約期間のバリデーション
     */
    public function validateContractPeriod(string $startDate, string $endDate = null)
    {
     ...
     * 請求日の妥当性チェック
     */
    public function validateBillingDay(int $billingDay)
    {
     ...
     * 支払い条件の妥当性チェック
     */
    public function validatePaymentTerms(int $paymentTerms)
    {
    ...
    {
        return $this->update($clientId, array('is_active' => $isActive));
    }

    /**
     * 契約終了間近のクライアントを取得
     */
    public function getExpiringContracts(int $daysAhead = 30)
    {
        ...
        return $this->processResults($this->query($sql, array($targetDate)));
    }
    ...
    public function getBillingClients()
    {
        ...
        return $this->processResults($this->query($sql, array($today)));
    }
}