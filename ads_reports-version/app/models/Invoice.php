...
class Invoice extends BaseModel
{
    ...
    public function generateInvoiceNumber()
    {
        ...
        $format = $configarray('billing')array('invoice_number_format') ?? 'INV-%Y%m%d-%04d';
        
        $datePrefix = date(str_replace(array('%Y', '%m', '%d'), array('Y', 'm', 'd'), $format));
        ...
        $result = $this->query($sql, array($likePattern . '____'));
        
        $nextSeq = ($resultarray(0)array('max_seq') ?? 0) + 1;
     ...
     * 請求書を作成
     */
    public function createInvoice(int $clientId, string $startDate, string $endDate)
    {
        $client = new Client();
        ...
        $config = require __DIR__ . '/../../config/app.php';
        $taxRate = $configarray('billing')array('default_tax_rate') ?? 0.10;
        $paymentTerms = $clientDataarray('payment_terms') ?? 30;
        ...
        return $this->transaction(function() use ($invoiceData, $billingData) {
            ...
            foreach ($billingData as $item) {
                $invoiceItemModel->create([
                    ...
                    'ad_account_id' => $itemarray('ad_account_id'),
                    'platform' => $itemarray('platform'),
                    'description' => $this->generateItemDescription($item),
                    'ad_cost' => $itemarray('ad_cost'),
                    'fee_amount' => $itemarray('fee_amount')
                ]);
            }

            // 月次集計データの請求フラグを更新
            $this->updateMonthlySummaryInvoiceFlag($billingData, $invoicearray('id'));
     ...
     * 請求データの集計
     */
    private function aggregateBillingData(int $clientId, string $startDate, string $endDate)
    {
        $sql = "SELECT 
                    ms.ad_account_id,
                    aa.platform,
     ...
     * 請求明細の説明文を生成
     */
    private function generateItemDescription(array $item)
    {
        $platform = match($itemarray('platform')) {
            'google_ads' => 'Google広告',
            'yahoo_display' => 'Yahoo!ディスプレイ広告',
            'yahoo_search' => 'Yahoo!検索広告',
            default => $itemarray('platform')
        };

        return "{$platform} ({$itemarray('account_name')}) - {$itemarray('months_count')}ヶ月分";
    }

    /**
     * 月次集計データの請求フラグを更新
     */
    private function updateMonthlySummaryInvoiceFlag(array $billingData, int $invoiceId)
    {
        ...
        foreach ($billingData as $item) {
            ...
            $stmt->execute(array($item['ad_account_id')]);
        }
    }
    ...
    public function issueInvoice(int $invoiceId): ?array
    {
        ...
        if (!$invoice || $invoicearray('status') !== 'draft') {
    ...
    public function cancelInvoice(int $invoiceId): ?array
    {
        ...
        if (!$invoice || $invoicearray('status') === 'paid') {
        ...
        return $this->transaction(function() use ($invoiceId) {
            ...
            return $this->update($invoiceId, array('status' => 'cancelled'));
        });
    }
    ...
    public function getOverdueInvoices()
    {
        ...
        return $this->processResults($this->query($sql, array($today, $today)));
    }

    /**
     * 今月の請求統計を取得
     */
    public function getMonthlyInvoiceStats(string $yearMonth = null)
    {
        ...
        $result = $this->query($sql, array($yearMonth));
     ...
     * クライアント別請求履歴を取得
     */
    public function getClientInvoiceHistory(int $clientId, int $limit = null)
    {
    ...
    {
        $invoiceItemModel = new InvoiceItem();
        return $invoiceItemModel->findAllBy(array('invoice_id' => $invoiceId));
    }
    ...
    public function getInvoiceWithDetails(int $invoiceId): ?array
    {
        ...
        $result = $this->query($sql, array($invoiceId));
        
        if (empty($result)) {
            return null;
        }

        $invoice = $this->processResult($resultarray(0));
        $invoicearray('items') = $this->getInvoiceItems($invoiceId);
     ...
     * 請求書の一括ステータス更新
     */
    public function bulkUpdateStatus(array $invoiceIds, string $status)
    {
        if (empty($invoiceIds)) {
            return 0;
        }

        $allowedStatuses = array('draft', 'sent', 'paid', 'overdue', 'cancelled');
