...
{
    protected string $table = 'cost_markups';
     ...
     */
    public function getActiveMarkupsForAccount(int $accountId): array
    {
        $today = date('Y-m-d');
        ...
        return $this->processResults($this->query($sql, [$accountId, $accountId, $today, $today]));
    }

    /**
     * 特定日時のアカウント上乗せ設定を取得
     */
    public function getActiveMarkupsForAccountOnDate(int $accountId, string $date): array
    {
        $sql = "SELECT * FROM {$this->table}
                WHERE (ad_account_id = ? OR (ad_account_id IS NULL AND client_id IN (
                    SELECT client_id FROM ad_accounts WHERE id = ?
        ...
        return $this->processResults($this->query($sql, [$accountId, $accountId, $date, $date]));
    }

    /**
     * クライアント別の上乗せ設定を取得
     */
    public function getMarkupsForClient(int $clientId): array
    {
        return $this->findAllBy(['client_id' => $clientId], ['effective_from' => 'DESC']);
    }

    /**
     * 上乗せ費用を計算
     */
    public function calculateMarkup(int $accountId, float $originalCost, string $date = null): array
    {
        ...
        foreach ($markups as $markup) {
            ...
            switch ($markup['markup_type']) {
                case 'percentage':
                    $markupAmount = $originalCost * ($markup['markup_value'] / 100);
                    break;
                    
                case 'fixed':
                    $markupAmount = $markup['markup_value'];
                    break;
            }
     ...
     * 上乗せ設定の有効性チェック
     */
    public function validateMarkup(array $data): array
    {
        $errors = [];
        ...
        if (!isset($data['markup_value']) || $data['markup_value'] < 0) {
            $errors[] = '上乗せ値は0以上で入力してください';
        }

        // 上乗せタイプ別チェック
        if ($data['markup_type'] === 'percentage') {
            if ($data['markup_value'] > 100) {
                $errors[] = 'パーセンテージは100%以下で入力してください';
            }
        }
        ...
        if (!empty($data['ad_account_id'])) {
            ...
            $account = $adAccountModel->find($data['ad_account_id']);
            
            if (!$account) {
                $errors[] = '指定された広告アカウントが存在しません';
            } elseif ($account['client_id'] != $data['client_id']) {
                $errors[] = '広告アカウントとクライアントが一致しません';
            }
        }
     ...
     * クライアントの上乗せ履歴を取得
     */
    public function getClientMarkupHistory(int $clientId): array
    {
        $sql = "SELECT 
                    cm.*,
                    aa.account_name,
     ...
     * 上乗せ統計を取得
     */
    public function getMarkupStats(): array
    {
        $sql = "SELECT 
                    markup_type,
     ...
     * 月別上乗せ効果レポートを取得
     */
    public function getMonthlyMarkupReport(string $yearMonth): array
    {
        $sql = "SELECT 
                    c.company_name,
                    aa.account_name,
        ...
        return $this->query($sql, [$yearMonth]);
    }

    /**
     * 日別上乗せ計算の実行
     */
    public function applyDailyMarkups(string $date): int
    {
        ...
        $dailyData = $this->query($sql, [$date]);
        $updatedCount = 0;

        foreach ($dailyData as $data) {
            $markup = $this->calculateMarkup($data['ad_account_id'], $data['cost'], $date);
            
            if ($markup['total_cost'] != $data['reported_cost']) {
                $dailyDataModel->update($data['id'], [
                    'reported_cost' => $markup['total_cost']
                ]);
                $updatedCount++;
            }
        }
     ...
     */
    public function applyBackdatedMarkup(int $markupId, string $startDate, string $endDate): int
    {
        ...
        if ($markup['ad_account_id']) {
            $accountIds = [$markup['ad_account_id']];
        } else {
            $adAccountModel = new AdAccount();
            $accounts = $adAccountModel->getActiveAccountsByClient($markup['client_id']);
        ...
        foreach ($accountIds as $accountId) {
            ...
            $dailyData = $this->query($sql, [$accountId, $startDate, $endDate]);
            
            foreach ($dailyData as $data) {
                $newMarkup = $this->calculateMarkup($accountId, $data['cost'], $data['date_value']);
                
                if ($newMarkup['total_cost'] != $data['reported_cost']) {
                    $dailyDataModel->update($data['id'], [
                        'reported_cost' => $newMarkup['total_cost']
                    ]);
                    $updatedCount++;
                }
            }
     ...
     * 上乗せ設定の複製
     */
    public function duplicateMarkup(int $markupId, array $overrides = []): ?array
    {
        ...
        // 新しい有効期間を設定
        if (!isset($overrides['effective_from'])) {
            $newData['effective_from'] = date('Y-m-d');
        }
